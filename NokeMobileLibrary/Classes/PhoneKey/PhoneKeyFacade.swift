            ...
            do {
                ...
                if let phoneKeyInfo = try self.persistence.getPhoneKeyInfo(userId: userId) {
                    ...
                    DispatchQueue.main.async {
                        let binaries = cachedAcls.map {
                            return $0.sanitizedAclJSONString()
                        }
                        print("Binaries: \(binaries)")
                        print("Cached acls: \(cachedAcls)")
...
public extension BulkPhoneKeyAcl {
    private func sanitizeBase64(_ input: String) -> String {
        var s = input.trimmingCharacters(in: .whitespacesAndNewlines)

        // Remove all whitespace/newlines inside the string
    ...
    func sanitizedAclJSONString() -> String? {
        let sanitizedBase64 = sanitizeBase64(self.aclBinary)

        guard let binaryData = Data(base64Encoded: sanitizedBase64) else {
            print("[BulkPhoneKeyAcl] Error: ACL binary is not valid Base64 after sanitization")
            return nil
        }

        // Remove UTF‑8 BOM if present
        let cleanedData: Data
        if binaryData.starts(with: [0xEF, 0xBB, 0xBF]) {
            cleanedData = binaryData.dropFirst(3)
        } else {
            cleanedData = binaryData
        }

        // Validate JSON structure
        let jsonString = String(data: cleanedData, encoding: .utf8)

        print("JSON acl: \(jsonString)")
        return jsonString
    }
}
