...
internal final class Ion2SigningCoordinator {
    ...
    func handleStatus(_ status: NokeDeviceSigningStatus) {
        ...
        case .unlockExecuted, .lockAlreadyUnlocked:
            handleUnlockExecuted()
            characteristicHandler.readStatus()
        ...
        case .lockJammed:
            fail(.lockJammed)
        case .cmdSigVerifyFailed:
            fail(.commandSignatureFailed)
        default:
    ...
    func reset(shouldResetHandlers: Bool = false) {
        state = .idle
        currentAcl = nil
        currentAclData = nil
    ...
    private func isStateAtRest() -> Bool {
        switch state {
        case .idle, .completed, .failed:
            return true
        default:
    ...
    private func isReset() -> Bool {
        return isStateAtRest() && currentAcl == nil && currentAclData == nil && currentSignatureData == nil && currentCommandSignatureData == nil && userId == nil && deviceID == nil && retryCount == 0 && successHandler == nil && failureHandler == nil
    }
    ...
    private func handleUnlockExecuted() {
        ...
        reset(shouldResetHandlers: true)
    }
    
    private func handleOutOfSchedule() {
        fail(.outOfSchedule)
    }
